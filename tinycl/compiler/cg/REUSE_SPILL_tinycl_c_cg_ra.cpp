...
//
// @(#)$Id: //proj/evedit2/mainline/tinycl/compiler/cg/tinycl_c_cg_ra.cpp#25 $
//
#include "./tinycl_c_cg.h"

#pragma warning(disable: 4355) // 'this' : used in base member initializer list

#define REUSE_SPILL 0
    ...
    protected InstructionDispatcher
{
    ...
    #if REUSE_SPILL
        private: WorkList_<StackSlot> m_oFreeStackSlots;
    #if REUSE_SPILL
    ...
    private: void releaseMx(Register* const pRx)
    {
        #if REUSE_SPILL
            if (StackSlot* const pMx = pRx->m_pSpill)
            {
                CLOG(3, "<li>release ~S</li>", pMx);
                m_oFreeStackSlots.Push(pMx);
            }
        #endif
    } // releaseMx
    ...
    private: void restoreStackSlots(BBlock* const pBB)
    {
        ASSERT(NULL != pBB);
        ...
        class Local
        {
            public: static void DeleteStackSlot(
                int                     const iLoc,
                WorkList_<StackSlot>*   const pStackSlots )
            {
                WorkList_<StackSlot> oTemp;
                while (! pStackSlots->IsEmpty())
                {
                    StackSlot* const pMx = pStackSlots->Pop();
                    if (pMx->GetLocation() == iLoc)
                    {
                        CLOG(3, "<li>restore ~S</li>", pMx);
                    }
                    else
                    {
                        oTemp.Push(pMx);
                ...
                while (! oTemp.IsEmpty())
                {
                    pStackSlots->Push(oTemp.Pop());
                }
            } // DeleteStackSlot
        }; // Local
        
        CLOG_SECTION(3, "restore stack slots for ~S", pBB);

        foreach (RegList::Enum, oEnum, m_pRegList)
        {
            Register* const pRx = oEnum.Get();
            if (StackSlot* pMx = pRx->m_pSpill)
            {
                if (pBB->IsLiveOut(pRx->GetIndex()))
                {
                    Local::DeleteStackSlot(
                        pMx->GetLocation(),
                        &m_oFreeStackSlots );
                }
            }
    ...
    DefProcI(Call)
    {
        ...
        while (! oFrees.IsEmpty())
        {
            releaseMx(oFrees.Pop());
        }
